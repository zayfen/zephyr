...
class Iterator {
  ...
  public lookforward (num: number): string {
    return this.source.slice(this._position, this._position + num)
  }

  public peekTo (reg: RegExp): string {
    // this.position-1是因为当前的_position指向的是char1,我们需要从char0开始截取
    let sliceStr: string = this.source.slice(this._position - 1)
...
export default class Lexer  {
  ...
  peekTo (reg: RegExp): string {
    let s = this.chars.peekTo(reg)
  ...
  private parseText (): Token {
    ...
    let value = this.peekTo(endTagReg)

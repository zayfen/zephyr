...
export class Zephyr {
  ...
  private mutationCallback = function (
    ...
    mutations.forEach((mutation) => {
      ...
      if (mutation.addedNodes.length > 0) {
        // 增加了节点, 此时我们认为挂载成功,可以调用onMounted回调函数
        this.onMounted();
      }
    });
  };
  ...
  /**
   * to install a component by component installer
   * @param { (ComponentAssets) => void } installer component installer
   */
  ...
  /**
   * 使用的Layout
   * @param layout {string} - ComponentAssets中注册的layout的名字
   */
  ...
  /**
   * 使用主题
   * @param theme {string} - ComponentAssets中注册的主题的名字
   */
  ...
  private injectLayoutAndTheme() {
    const layoutManager: LayoutManager = this.assets.findLayoutManager(
      this.layout
    );
  ...
  public mount(target: string | Element, node?: VNode) {
    ...
    if (typeof target === 'string') {
      ...
      _target = document.getElementById(
        target[0] === '#' ? target.substr(1) : target
      );
  ...
  onMounted() {
    ...
    function allChildrenVisited(node: VNode) {
      return node.children.reduce(
        (prevVal: boolean, child: VNode) =>
          prevVal && visitedNodesCache[child.id],
        true
      );
    }
